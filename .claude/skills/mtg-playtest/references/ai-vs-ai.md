...
1. 対象の2デッキだけ `deck show <登録名> --brief` で確認する。登録JSON全文は読まない。BO1はメインの名前・枚数・警告で足りる。登録済みで変更がなければ再登録・全件検証しない。新規・変更時は `deck add <file> --quiet` で検証し、警告を解消する。カードキャッシュ全体、過去対局、履歴、ソース全文は読まない。
